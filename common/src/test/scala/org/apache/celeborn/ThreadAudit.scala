...
trait ThreadAudit extends Logging {
  ...
  val threadExcludeList = Set(
    ...
    /**
     * During creation [[org.apache.celeborn.common.rpc.netty.NettyRpcEnv]]
     * creates event loops. One is wrapped inside
     * [[org.apache.celeborn.common.network.server.TransportServer]]
     * the other one is inside [[org.apache.celeborn.common.network.client.TransportClient]].
     * Calling [[org.apache.celeborn.common.rpc.netty.NettyRpcEnv#shutdown]] will shut down the
  ...
  protected def doThreadPostAudit(): Unit = {
    val shortSuiteName = this.getClass.getName.replaceAll("org.apache.celeborn", "c.a.e")

    if (threadNamesSnapshot.nonEmpty) {
      val remainingThreadNames = runningThreadNames().diff(threadNamesSnapshot)
        .filterNot { s => threadExcludeList.exists(s.matches) }
