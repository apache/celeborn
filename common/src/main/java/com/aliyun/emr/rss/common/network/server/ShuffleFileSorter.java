...
public class ShuffleFileSorter {
  ...
  public void sort() {
    ...
    try {
      ...
      logger.error("Shuffle sort file failed ,{}", e);
    } finally {
      tracker.releaseSortMemory(fileLen);
    }
  }

  public FileInfo resolve(int startMapId, int endMapId) {
    try {
      ...
      return new FileInfo(new File(this.sortedFile),
        refreshChunkOffsets(startMapId, endMapId, indexMap));
    } catch (Exception e) {
  ...
  private ArrayList<Long> refreshChunkOffsets(int startMapId, int endMapId,
    Map<Integer, List<ShuffleBlockInfo>> indexMap) {
    ArrayList<Long> sortedChunkOffset = new ArrayList<>();
    ShuffleBlockInfo lastBlock = null;
    logger.debug("refresh offsets for file {} , startMapId {} endMapId {}",
      this.sortedFile, startMapId, endMapId);
