    ...
    this match {
      ...
      case RegisterWorker(host, rpcPort, pushPort, fetchPort, replicatePort, disks, requestId) =>
        val pbDisks = disks.asScala
          ...
          .toMap
          .asJava
        val payload = TransportMessages.PbRegisterWorker
          .newBuilder()
          .setHost(host)
          .setRpcPort(rpcPort)
      ...
      case HeartbeatFromWorker(
        ...
        val pbDisks = disks.asScala
          ...
          .toMap
          .asJava
        val payload = TransportMessages.PbHeartbeatFromWorker
          .newBuilder()
          .setHost(host)
          .setRpcPort(rpcPort)
          .setPushPort(pushPort)
      ...
      case HeartbeatResponse(expiredShuffleKeys, registered) =>
        val payload = TransportMessages.PbHeartbeatResponse
          .newBuilder()
      ...
      case RegisterShuffle(applicationId, shuffleId, numMappers, numPartitions) =>
        val payload = TransportMessages.PbRegisterShuffle
          .newBuilder()
      ...
      case RegisterShuffleResponse(status, partitionLocations) =>
        val builder = TransportMessages.PbRegisterShuffleResponse
          .newBuilder()
      ...
      case ReleaseSlots(applicationId, shuffleId, workerIds, slots, requestId) =>
        val payload = TransportMessages.PbReleaseSlots
          .newBuilder()
          .setApplicationId(applicationId)
          .setShuffleId(shuffleId)
          .addAllSlots(slots)
      ...
      case ReleaseSlotsResponse(status) =>
        val payload = TransportMessages.PbReleaseSlotsResponse
          .newBuilder()
          .setStatus(status.getValue)
          .build()
          .toByteArray
      ...
      case RequestSlotsResponse(status, workerResource) =>
        val builder = TransportMessages.PbRequestSlotsResponse
          .newBuilder()
          .setStatus(status.getValue)
      ...
      case ChangeLocationResponse(status, location) =>
        val builder = TransportMessages.PbChangeLocationResponse
          .newBuilder()
          .setStatus(status.getValue)
        if (location != null) {
      ...
      case MapperEnd(applicationId, shuffleId, mapId, attemptId, numMappers) =>
        val payload = TransportMessages.PbMapperEnd
          .newBuilder()
          .setApplicationId(applicationId)
          .setShuffleId(shuffleId)
          .setMapId(mapId)
      ...
      case MapperEndResponse(status) =>
        val payload = TransportMessages.PbMapperEndResponse
          .newBuilder()
          .setStatus(status.getValue)
          .build()
          .toByteArray
      ...
      case GetReducerFileGroup(applicationId, shuffleId) =>
        val payload = TransportMessages.PbGetReducerFileGroup
          .newBuilder()
          .setApplicationId(applicationId)
          .setShuffleId(shuffleId)
          .build()
          ...
          fileGroup
            .map { arr =>
              PbFileGroup
                .newBuilder()
                .addAllLocaltions(
                  arr.map(PartitionLocation.toPbPartitionLocation).toIterable.asJava
                )
                .build()
            }
            .toIterable
      ...
      case WorkerLost(host, rpcPort, pushPort, fetchPort, replicatePort, requestId) =>
        val payload = TransportMessages.PbWorkerLost
          .newBuilder()
          .setHost(host)
          .setRpcPort(rpcPort)
          .setPushPort(pushPort)
      ...
      case WorkerLostResponse(success) =>
        val payload = TransportMessages.PbWorkerLostResponse
          .newBuilder()
          .setSuccess(success)
          .build()
          .toByteArray
      ...
      case StageEnd(applicationId, shuffleId) =>
        val payload = TransportMessages.PbStageEnd
          .newBuilder()
          .setApplicationId(applicationId)
          .setShuffleId(shuffleId)
          .build()
      ...
      case StageEndResponse(status) =>
        val payload = TransportMessages.PbStageEndResponse
          .newBuilder()
          .setStatus(status.getValue)
          .build()
          .toByteArray
      ...
      case UnregisterShuffle(appId, shuffleId, requestId) =>
        val payload = TransportMessages.PbUnregisterShuffle
          .newBuilder()
          .setAppId(appId)
          .setShuffleId(shuffleId)
          .setRequestId(requestId)
      ...
      case UnregisterShuffleResponse(status) =>
        val payload = TransportMessages.PbUnregisterShuffleResponse
          .newBuilder()
          .setStatus(status.getValue)
          .build()
          .toByteArray
      ...
      case ApplicationLost(appId, requestId) =>
        val payload = TransportMessages.PbApplicationLost
          .newBuilder()
          .setAppId(appId)
          .setRequestId(requestId)
          .build()
      ...
      case ApplicationLostResponse(status) =>
        val payload = TransportMessages.PbApplicationLostResponse
          .newBuilder()
          .setStatus(status.getValue)
          .build()
          .toByteArray
      ...
      case HeartBeatFromApplication(appId, totalWritten, fileCount, requestId) =>
        val payload = TransportMessages.PbHeartBeatFromApplication
          .newBuilder()
          .setAppId(appId)
          .setRequestId(requestId)
          .build()
      ...
      case GetBlacklist(localBlacklist) =>
        val payload = TransportMessages.PbGetBlacklist
          .newBuilder()
          .addAllLocalBlackList(localBlacklist.asScala.map(WorkerInfo.toPbWorkerInfo).toList.asJava)
          .build()
          .toByteArray
      ...
      case GetBlacklistResponse(statusCode, blacklist, unknownWorkers) =>
        val builder = TransportMessages.PbGetBlacklistResponse
          .newBuilder()
      ...
      case GetClusterLoadStatus(numPartitions) =>
        new TransportMessage(
          TransportMessages.MessageType.GET_CLUSTER_LOAD_STATUS,
      ...
      case GetClusterLoadStatusResponse(isOverload) =>
        val payload = TransportMessages.PbGetClusterLoadStatusResponse
          .newBuilder()
          .setIsOverload(isOverload)
          .build()
          .toByteArray
        new TransportMessage(
          TransportMessages.MessageType.GET_CLUSTER_LOAD_STATUS_RESPONSE,
          payload
        )

      case ReportWorkerFailure(failed, requestId) =>
        val payload = TransportMessages.PbReportWorkerFailure
          .newBuilder()
      ...
      case RegisterWorkerResponse(success, message) =>
        val payload = TransportMessages.PbRegisterWorkerResponse
          .newBuilder()
          .setSuccess(success)
          .setMessage(message)
          .build()
      ...
      case ReregisterWorkerResponse(success) =>
        val payload = TransportMessages.PbReregisterWorkerResponse
          .newBuilder()
          .setSuccess(success)
          .build()
          .toByteArray
        ...
        val payload = TransportMessages.PbReserveSlots
          ...
          .addAllMasterLocations(
            masterLocations.asScala
              .map(PartitionLocation.toPbPartitionLocation(_))
              .toList
              .asJava
          )
          .addAllSlaveLocations(
            slaveLocations.asScala
              .map(PartitionLocation.toPbPartitionLocation(_))
              .toList
              .asJava
          )
      ...
      case ReserveSlotsResponse(status, reason) =>
        val payload = TransportMessages.PbReserveSlotsResponse
          .newBuilder()
          .setStatus(status.getValue)
          .setReason(reason)
          .build()
      ...
      case CommitFiles(applicationId, shuffleId, masterIds, slaveIds, mapAttempts) =>
        val payload = TransportMessages.PbCommitFiles
          .newBuilder()
          .setApplicationId(applicationId)
          .setShuffleId(shuffleId)
      ...
      case Destroy(shuffleKey, masterLocations, slaveLocations) =>
        val payload = TransportMessages.PbDestroy
          .newBuilder()
      ...
      case DestroyResponse(status, failedMasters, failedSlaves) =>
        val builder = TransportMessages.PbDestroyResponse
          .newBuilder()
          .setStatus(status.getValue)
      ...
      case SlaveLostResponse(status, slaveLocation) =>
        val payload = TransportMessages.PbSlaveLostResponse
          .newBuilder()
          .setStatus(status.getValue)
      ...
      case GetWorkerInfosResponse(status, workerInfos @ _*) =>
        val payload = TransportMessages.PbGetWorkerInfosResponse
          .newBuilder()
          .setStatus(status.getValue)
      ...
      case ThreadDumpResponse(threadDump) =>
        val payload = TransportMessages.PbThreadDumpResponse
          .newBuilder()
...
object ControlMessages extends Logging {
  ...
  case class HeartbeatFromWorker(
      ...
      fetchPort: Int,
      replicatePort: Int,
      slots: util.Map[String, DiskInfo],
      shuffleKeys: util.HashSet[String],
      override var requestId: String = ZERO_UUID
  ) extends MasterRequestMessage

  case class HeartbeatResponse(expiredShuffleKeys: util.HashSet[String], registered: Boolean)
  ...
  case class ReleaseSlotsResponse(status: StatusCode) extends MasterMessage

  case class RequestSlotsResponse(status: StatusCode, workerResource: WorkerResource)
    extends MasterMessage
  ...
  case class ChangeLocationResponse(status: StatusCode, partition: PartitionLocation)
    extends MasterMessage
  ...
  case class StageEndResponse(status: StatusCode) extends MasterMessage
  ...
  case class ApplicationLost(appId: String, override var requestId: String = ZERO_UUID)
    extends MasterRequestMessage
  ...
  def fromTransportMessage(message: TransportMessage): Message = {
    message.getType match {
      ...
      case REGISTER_WORKER =>
        ...
        val disks = pbRegisterWorker.getDisksMap.asScala
          .map(item =>
            item._1 ->
              new DiskInfo(
                item._1,
      ...
      case REGISTER_SHUFFLE_RESPONSE =>
        ...
        if (pbRegisterShuffleResponse.getPartitionLocationsCount > 0) {
          partitionLocations.addAll(
            pbRegisterShuffleResponse.getPartitionLocationsList.asScala
      ...
      case REPORT_WORKER_FAILURE =>
        ...
        ReportWorkerFailure(
          new util.ArrayList[WorkerInfo](
            pbReportWorkerFailure.getFailedList.asScala
              .map(WorkerInfo.fromPbWorkerInfo(_))
              .toList
              .asJava
          ),
      ...
      case PARTITION_SPLIT =>
        ...
        val partition = if (pbShuffleSplitRequest.hasOldPartition) {
