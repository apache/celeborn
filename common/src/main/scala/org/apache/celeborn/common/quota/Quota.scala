...
import org.apache.celeborn.common.identity.{Identifier, UserIdentifier}
...
case class Quota(
  ...
  def update(identifier: Identifier, name: String, value: Long): Unit = {
    name match {
      ...
      case _ => logWarning(s"Unsupported quota name: $name for user: $identifier.")
    }
  }
  ...
  private def checkDiskFileCount(identifier: Identifier, value: Long): (Boolean, String) = {
  ...
  private def checkHdfsFileCount(identifier: Identifier, value: Long): (Boolean, String) = {
