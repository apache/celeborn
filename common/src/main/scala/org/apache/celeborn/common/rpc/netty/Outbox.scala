...
private[celeborn] case class OneWayOutboxMessage(content: ByteBuffer) extends OutboxMessage
  with Logging {
...
private[celeborn] case class RpcOutboxMessage(
    content: ByteBuffer,
...
private[celeborn] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
