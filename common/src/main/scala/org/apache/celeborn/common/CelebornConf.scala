...
class CelebornConf(loadDefaults: Boolean) extends Cloneable with Logging with Serializable {
  ...
  def clientPushUnsafeRowFastWrite: Boolean = get(CLIENT_PUSH_UNSAFEROW_FASTWRITE)
...
object CelebornConf extends Logging {
  ...
  val CLIENT_PUSH_UNSAFEROW_FASTWRITE: ConfigEntry[Boolean] =
    buildConf("celeborn.push.unsafeRow.fastWrite")
      .categories("client")
      .version("0.2.2")
