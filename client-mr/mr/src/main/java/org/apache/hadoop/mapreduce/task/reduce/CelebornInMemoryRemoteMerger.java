...
public class CelebornInMemoryRemoteMerger<K, V> extends MergeThread<InMemoryMapOutput<K, V>, K, V> {
  private static final Logger LOG = LoggerFactory.getLogger(CelebornInMemoryRemoteMerger.class);

  private static final String SPILL_OUTPUT_PREFIX = "spill";
  private final CelebornRemoteSpillMergeManager<K, V> manager;
  ...
  public CelebornInMemoryRemoteMerger(
          CelebornRemoteSpillMergeManager<K, V> manager,
      JobConf jobConf,
      FileSystem remoteFs,
      Path spillPath,
      String taskId,
  ...
  public void merge(List<InMemoryMapOutput<K, V>> inputs) throws IOException {
    ...
    // 将inputs转换为Merger.Segment列表，便于后续合并
    createInMemorySegments(inputs, inMemorySegments);
    int noInMemorySegments = inMemorySegments.size();

    // 创建hdfs输出路径： SPILL_OUTPUT_PREFIX: spill , SEPARATOR: /
    // spill/prefix/taskAttemptId/mapId
    ...
    try {
      ...
      // 执行内数据归并排序，调用的是原生的多了归并算法
      // 输入： 多个已排序的内存segments
      // 输出：RawKeyValueIterator，按key排序的数据流
      // 这里是对内存中的多个Segments进行归并，不是磁盘文件
      rIter =
          Merger.merge(
              jobConf,
              remoteFs,
  ...
  private long createInMemorySegments(
    ...
    long fullSize = 0L;// 所有InMemoryMapOutput的总字节数
    for (InMemoryMapOutput<K, V> mo : inMemoryMapOutputs) {
      fullSize += mo.getMemory().length;// 累加所有内存数据的大小
    }
    while (fullSize > 0) {
      // 从列表移除元素，而不是复制
