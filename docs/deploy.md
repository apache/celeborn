...
Copy `$CELEBORN_HOME/spark/<celeborn-client-spark-{version} Jar>` to `$SPARK_HOME/jars/`.
...
Copy `$CELEBORN_HOME/flink/<celeborn-client-flink-{version} Jar>` to `$FLINK_HOME/lib/`.
...
Copy `$CELEBORN_HOME/mr/<celeborn-client-mr Jar>` into `mapreduce.application.classpath` and `yarn.application.classpath`.
