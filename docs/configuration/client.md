...
| celeborn.client.spark.push.dynamicWriteMode.enabled | false | Whether to dynamically switch push write mode based on conditions.If true, shuffle mode will be is determined by partition count,not celeborn.client.spark.shuffle.writer | 0.4.0 | 
| celeborn.client.spark.push.dynamicWriteMode.partitionNum.threshold | 2000 | Threshold of shuffle partition count for push writer mode by partition count.If celeborn.client.spark.push.dynamicWriteMode.enabled is true, Celeborn will use sort-based shuffle writer when partition count is greater than this value,and used hash-based shuffle writer otherwise. | 0.4.0 | 
