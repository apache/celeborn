...
| celeborn.client.spark.push.sort.memory.useAdaptiveThreshold | false | false | adaptively adjust threshold for sort shuffle writer's memory threshold | 0.5.0 | celeborn.push.sortMemory.useAdaptiveThreshold | 
| celeborn.client.spark.push.sort.smallPushTolerateFactor | 0.2 | false | only be in effect when celeborn.client.spark.push.sort.memory.adaptiveThreshold is turned on. It controls when to enlarge the sort shuffle writer's memory threshold. With N bytes data in memory and V as the value of this config, if the number of pushes, C, when using sort based shuffle writer C >= (1 + V) * C' where C' is the number of pushes if we were using hash based writer, we will enlarge the memory threshold by 2X. | 0.5.0 | celeborn.push.sortMemory.adaptiveThreshold | 
