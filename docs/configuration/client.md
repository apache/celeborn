...
| celeborn.client.spark.push.writer.modeByPartitionCount.enable | false | Whether to enable push writer mode by partition count. If true, Celeborn will use sort-based shuffle writer when shuffle partition count is greater than `celeborn.client.spark.push.writer.modeByPartitionCount.threshold`. | 0.4.0 | 
