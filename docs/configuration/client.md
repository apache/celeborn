...
| celeborn.push.limit.strategy | SIMPLE | The strategy used to control the push speed. | 0.3.0 | 
...
| celeborn.push.slowStart.maxSleepTime | 60s | If celeborn.push.limit.strategy is set to SLOWSTART, push side will take a sleep strategy for each batch of requests, this controls the max sleep time if the max in flight requests limit is 1 for a long time | 0.3.0 | 
