...
| celeborn.master.splitSlot.assign.maxWorkers | 500 | false | Max workers that split slots of the whole application's shuffle can be allocated on. Will choose the smaller positive one from Master side and Client side, see `celeborn.client.slot.assign.maxWorkers`. | 0.7.0 |  |
