...
| celeborn.worker.http.auth.supportedSchemes |  | false | A comma-separated list of worker http auth supported schemes.<ul> <li>SPNEGO: Kerberos/GSSAPI authentication.</li> <li>BASIC: User-defined password authentication, anonymous by default.</li> <li>BEARER: User-defined bearer token authentication, anonymous by default.</li></ul> | 0.5.0 |  | 
