...
| celeborn.master.endpoints.resolver | org.apache.celeborn.common.client.StaticMasterEndpointResolver | false | Resolver class that can be used for dynamically discovering and updating the master endpoints. This allows for a custom resolver implementation to be provided by the user. This is useful in environments where the master nodes might change due to scaling operations or infrastructure updates. | 0.6.0 |  | 
