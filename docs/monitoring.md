...
These metrics are exposed by Celeborn worker.
  ...
  - namespace=worker
    ...
    | FetchDirectFileRegionCount             | The count of responses emitted through the direct FileRegion fast path.                                         |
    | FetchDirectFileRegionSize              | The payload size of responses emitted through the direct FileRegion fast path.                                  |
    | ActiveChunkStreamCount                 | Active stream count for reduce partition reading streams.                                                       |
    | ChunksBeingTransferred                 | The number of chunk responses currently accepted for send but not yet completed on the worker write path.       |
