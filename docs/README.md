...
`jars/` directory:
```shell
cp $CELEBORN_HOME/spark/<celeborn-client-spark-{version} Jar> $SPARK_HOME/jars/
```
#### Start spark-shell
...
`lib/` directory:
```shell
cp $CELEBORN_HOME/flink/<celeborn-client-flink-{version} Jar> $FLINK_HOME/lib/
```
...
```shell
cp $CELEBORN_HOME/mr/<celeborn-client-mr Jar> <mapreduce.application.classpath>
cp $CELEBORN_HOME/mr/<celeborn-client-mr Jar> <yarn.application.classpath>
```
