...
| /exit                      | POST   | type=${EXIT_TYPE}&timeout=${TIMEOUT}          | Trigger this worker to exit. Legal `type`s are 'Decommission', 'Graceful' and 'Immediately'. Optional `timeout` (e.g. 600s/30m/1h) overrides `celeborn.worker.decommission.forceExitTimeout` for this single Decommission exit; ignored for other types. |
