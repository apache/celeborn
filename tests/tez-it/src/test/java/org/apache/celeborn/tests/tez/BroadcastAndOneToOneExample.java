...
public class BroadcastAndOneToOneExample extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (doLocalityCheck
        && getConf()
            .getBoolean(TezConfiguration.TEZ_LOCAL_MODE, TezConfiguration.TEZ_LOCAL_MODE_DEFAULT)) {
  ...
  public boolean run(Configuration conf, boolean doLocalityCheck) throws Exception {
    ...
    String stagingDirStr =
        tezConf.get(
                TezConfiguration.TEZ_AM_STAGING_DIR, TezConfiguration.TEZ_AM_STAGING_DIR_DEFAULT)
            + Path.SEPARATOR
  ...
  private DAG createDAG(
      FileSystem fs, TezConfiguration tezConf, Path stagingDir, boolean doLocalityCheck)
    ...
    UnorderedKVEdgeConfig edgeConf =
        UnorderedKVEdgeConfig.newBuilder(Text.class.getName(), IntWritable.class.getName())
    ...
    dag.addVertex(inputVertex)
        ...
        .addEdge(
            Edge.create(
                broadcastVertex, oneToOneVertex, edgeConf.createDefaultBroadcastEdgeProperty()));
    return dag;
  }
    ...
    public void run() throws Exception {
      ...
      if (userPayload != null) {
        boolean doLocalityCheck =
            getContext().getUserPayload().getPayload().get(0) > 0 ? true : false;
        if (doLocalityCheck) {
    ...
    public void run() throws Exception {
      ...
      if (doLocalityCheck) {
        ...
        if (index == null || Integer.parseInt(index) != getContext().getTaskIndex()) {
          String msg =
              "Did not find expected local producer "
                  + getContext().getTaskIndex()
                  + " in the same JVM";
