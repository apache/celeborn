...
public class MapPartitionData implements MemoryManager.ReadBufferTargetChangeListener {
  ...
  public MapPartitionData(
    ...
    if (diskFileInfo.isDFS()) {
      ...
      this.indexInputStream = fileSystem.open(new Path(diskFileInfo.getIndexPath()));
      ...
      this.dataChannel = FileChannelUtils.openReadableFileChannel(diskFileInfo.getFilePath());
  ...
  protected void openReader(MapPartitionDataReader reader) throws IOException {
    reader.open(
        dataChannel, indexChannel, dataInputStream, indexInputStream, dataFileSize, indexFileSize);
  }
