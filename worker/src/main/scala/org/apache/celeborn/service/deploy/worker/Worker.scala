...
private[celeborn] class Worker(
  ...
  private val gracefulShutdown = CelebornConf.workerGracefulShutdown(conf)
  assert(
  ...
  val replicateThreadPool = ThreadUtils.newDaemonCachedThreadPool(
    ...
    CelebornConf.workerReplicateThreads(conf))
  val commitThreadPool = ThreadUtils.newDaemonCachedThreadPool(
    "Worker-CommitFiles",
    CelebornConf.workerCommitThreads(conf))
  val asyncReplyPool = ThreadUtils.newDaemonSingleThreadScheduledExecutor("async-reply")
  val timer = new HashedWheelTimer()

  // Configs
  private val HEARTBEAT_MILLIS = CelebornConf.workerHeartbeatTimeoutMs(conf) / 4
  ...
  ShutdownHookManager.get().addShutdownHook(
    new Thread(new Runnable {
      override def run(): Unit = {
        ...
        shutdown.set(true)
        if (gracefulShutdown) {
          val interval = CelebornConf.checkSlotsFinishedInterval(conf)
          val timeout = CelebornConf.checkSlotsFinishedTimeoutMs(conf)
          var waitTimes = 0
