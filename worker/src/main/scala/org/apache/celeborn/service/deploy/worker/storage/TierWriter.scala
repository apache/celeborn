...
abstract class TierWriterBase(
  ...
  def writerInternal(buf: ByteBuf): Unit
...
//          notifier.numPendingFlushes.incrementAndGet()
          ...
          if (numBytes > chunkSize) {
            ...
            while (dupBuf.isReadable) {
              ...
              fileInfo.updateBytesFlushed(compressedSize + 16)
            }
            dupBuf.release
          } else fileInfo.updateBytesFlushed(numBytes)
        } else {
          notifier.checkException()
    ...
    metaHandler.afterFlush(finalFlush)
  }
...
class MemoryTierWriter(
  ...
  override def writerInternal(buf: ByteBuf): Unit = {
    ...
    fileInfo.updateBytesFlushed(buf.readableBytes())
  }
...
class LocalTierWriter(
  ...
  override def writerInternal(buf: ByteBuf): Unit = {
    ...
    if (flushBufferReadableBytes != 0 && flushBufferReadableBytes + buf.readableBytes() >= flusherBufferSize) {
      flush(false)
    }
    buf.retain()
    try {
      ...
      case oom: OutOfMemoryError =>
        buf.release()
        MemoryManager.instance().releaseDiskBuffer(buf.readableBytes())
        throw oom;
    }
  }
...
class DfsTierWriter(
  ...
  override def closeStreams(): Unit = {
    if (hadoopFs.exists(hdfsFileInfo.getDfsPeerWriterSuccessPath)) {
      ...
      for (offset <- hdfsFileInfo.getReduceFileMeta.getChunkOffsets) {
