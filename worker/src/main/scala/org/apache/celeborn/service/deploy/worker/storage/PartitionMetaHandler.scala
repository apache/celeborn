...
trait PartitionMetaHandler {
  ...
  def handleEvent(message: GeneratedMessageV3): PartialFunction[Any, Unit]

  def beforeWrite(bytes: ByteBuf): Unit

  def afterWrite(size: Int): Unit

  def afterFlush(finalFlush: Boolean): Unit
...
class MapPartitionMetaHandler(
  ...
  var indexChannel: FileChannel = null
  @volatile var isRegionFinished = true

  override def handleEvent(message: GeneratedMessageV3): PartialFunction[Any, Unit] = {
  ...
  override def afterFlush(finalFlush: Boolean): Unit = {}
...
class ReducePartitionMetaHandler(val rangeReadFilter: Boolean, val fileInfo: FileInfo)
  ...
  override def afterFlush(finalFlush: Boolean): Unit = {
    fileInfo.getReduceFileMeta.updateChunkOffset(fileInfo.getFileLength, finalFlush)
  }
  ...
  override def handleEvent(message: GeneratedMessageV3): PartialFunction[Any, Unit] = {
    case _ =>
      logger.error(s"Reduce partition meta handler should not receive any protobuf message")
  }
}
...
class SegmentMapPartitionMetaHandler(diskFileInfo: DiskFileInfo, notifier: FlushNotifier)
  ...
  private var subPartitionBufferIndex: Array[Int] = null
