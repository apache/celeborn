  ...
  test("monitor device usage metrics") {
    withObjectMocked[org.apache.celeborn.common.util.Utils.type] {
      ...
      val metrics1 = workerSource2.gauges().filter(
        _.name.startsWith(WorkerSource.DEVICE_OS_TOTAL_CAPACITY)).sortBy(_.name)
      val metrics2 = workerSource2.gauges().filter(
        _.name.startsWith(WorkerSource.DEVICE_OS_FREE_CAPACITY)).sortBy(_.name)
      val metrics3 = workerSource2.gauges().filter(
        _.name.startsWith(WorkerSource.DEVICE_CELEBORN_TOTAL_CAPACITY)).sortBy(_.name)
      val metrics4 = workerSource2.gauges().filter(
        _.name.startsWith(WorkerSource.DEVICE_CELEBORN_FREE_CAPACITY)).sortBy(_.name)

      assertEquals(s"${WorkerSource.DEVICE_OS_TOTAL_CAPACITY}_vda", metrics1.head.name)
      assertEquals(1395864371200L, metrics1.head.gauge.getValue)
      assertEquals(s"${WorkerSource.DEVICE_OS_TOTAL_CAPACITY}_vdb", metrics1.last.name)
      assertEquals(1932735283200L, metrics1.last.gauge.getValue)

      assertEquals(s"${WorkerSource.DEVICE_OS_FREE_CAPACITY}_vda", metrics2.head.name)
      assertEquals(1293858897920L, metrics2.head.gauge.getValue)
      assertEquals(s"${WorkerSource.DEVICE_OS_FREE_CAPACITY}_vdb", metrics2.last.name)
      assertEquals(1835024777216L, metrics2.last.gauge.getValue)

      assertEquals(s"${WorkerSource.DEVICE_CELEBORN_TOTAL_CAPACITY}_vda", metrics3.head.name)
      assertEquals(Int.MaxValue.toLong * 2, metrics3.head.gauge.getValue)
      assertEquals(s"${WorkerSource.DEVICE_CELEBORN_TOTAL_CAPACITY}_vdb", metrics3.last.name)
      ...
      assertEquals(s"${WorkerSource.DEVICE_CELEBORN_FREE_CAPACITY}_vda", metrics4.head.name)
      assertEquals(1024L * 2, metrics4.head.gauge.getValue)
      assertEquals(s"${WorkerSource.DEVICE_CELEBORN_FREE_CAPACITY}_vdb", metrics4.last.name)
