...
import org.apache.spark.listener.{CelebornShuffleEarlyCleanup, CelebornShuffleEarlyCleanupEvent, ShuffleStatsTrackingListener}
      ...
      override def run(): Unit = {
        while (!stopped) {
          ...
          allShuffleIds.asScala.foreach { shuffleId =>
            ...
            CelebornSparkContextHelper.eventLogger.foreach(e => {
              // This event records shuffle deletion time
              e.onOtherEvent(CelebornShuffleEarlyCleanupEvent(
                shuffleId,
