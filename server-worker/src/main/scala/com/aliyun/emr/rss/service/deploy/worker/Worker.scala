...
private[deploy] class Worker(
  ...
  private lazy val shuffleSortScheduler = {
    ...
    new ThreadPoolExecutor(sortLimit, sortLimit, 10L, TimeUnit.SECONDS,
  ...
  override def handlePushData(pushData: PushData, callback: RpcResponseCallback): Unit = {
    ...
    if (isMaster && fileWriter.shouldSplit()) {
      logInfo(s"[handlePushData] fileWriter ${fileWriter}" +
        s" has shuffle file to split. Shuffle split threshold ${fileWriter.splitThreshold()}")
  ...
  override def handlePushMergedData(
    ...
    if (isMaster && fileWriterWithPendingSplit.nonEmpty) {
      logWarning(s"[handlePushMergedData] fileWriter ${fileWriterWithPendingSplit}" +
        s" has shuffle file to split")
      callback.onSuccess(ByteBuffer.wrap(Array[Byte](StatusCode.ShuffleFileSplit.getValue)))
      return
    }
  ...
  override def handleOpenStream(shuffleKey: String, fileName: String,
    startMapId: Int, endMapId: Int): FileInfo = {
    logInfo(s"Open file $fileName for $shuffleKey with" +
      s" startMapId ${startMapId} endMapId ${endMapId}")
    // find FileWriter responsible for the data
        ...
        if (!shuffleBoundSortingShuffleFiles.contains(shuffleFileId)) {
          ...
          val sortFuture = shuffleSortScheduler.submit(new Callable[FileInfo] {
            override def call(): FileInfo = {
              ...
              val fileInfo = sorter.resolve(startMapId, endMapId)
          ...
          try {
            ...
            case e1: TimeoutException =>
              ...
              logError(s"[Worker] Sort file ${fileWriter.getFile.getAbsoluteFile} timeout, detail : ${e1}")
              null
            case e: ExecutionException =>
              logError(s"[Worker] Sort file ${fileWriter.getFile.getAbsoluteFile} failed, detail : ${e}")
              null
          }
        } else {
          ...
          new ShuffleFileSorter(fileWriter.getFile, RssConf.workerFetchChunkSize(conf),
            shuffleSorterAllocator)
            .resolve(startMapId, endMapId)
        }
      } else {
        new ShuffleFileSorter(fileWriter.getFile, RssConf.workerFetchChunkSize(conf),
          shuffleSorterAllocator)
