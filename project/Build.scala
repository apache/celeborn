...
object CelebornBuild extends PomBuild {
  ...
  val common = ProjectRef(buildLocation, "common")
  val master = ProjectRef(buildLocation, "master")


  val spark3Client = ProjectRef(buildLocation, "spark-3-shaded")

  val allProjects@Seq(
    client, service, worker, _*
  ) = Seq(
    "client", "service", "worker"
  ).map(ProjectRef(buildLocation, _)) ++ Seq(common, master, spark3Client)
  ...
  enable(CelebornCommon.settings)(common)
  enable(CelebornMaster.settings)(master)
  enable(CelebornSpark3Client.settings)(spark3Client)
