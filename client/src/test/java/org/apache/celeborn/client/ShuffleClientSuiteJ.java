...
public class ShuffleClientSuiteJ {
  ...
  public void testRegisterShuffleFailed() throws IOException, InterruptedException {
    ...
    try {
      ...
      assert e.getMessage().contains("Register shuffle failed for shuffle 1.");
      assert e.getCause().getMessage().contains("Slot not available for shuffle 1");
    }
    ...
    try {
      ...
      assert e.getMessage().contains("Register shuffle failed for shuffle 1.");
      assert e.getCause().getMessage().contains("Reserve slots failed for shuffle 1");
    }
    ...
    try {
      ...
      assert e.getMessage().contains("Register shuffle failed for shuffle 1.");
      assert e.getCause().getMessage().contains("Request slots failed for shuffle 1");
    }
  }
