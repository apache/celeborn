...
public class BufferPacker {
  ...
  private final BiConsumerWithException<ByteBuf, BufferHeader, InterruptedException> ripeBufferHandler;
  ...
  public void process(Buffer buffer, int subIdx) throws InterruptedException {
    ...
    if (cachedBuffer == null) {
      ...
      handleRipeBuffer(dumpedBuffer, targetSubIdx, buffer.getDataType(), buffer.isCompressed());
    } else {
      /**
      ...
      if (cachedBuffer.readableBytes() + buffer.readableBytes() - BufferUtils.HEADER_LENGTH_PREFIX
        ...
        handleRipeBuffer(dumpedBuffer, currentSubIdx, dumpedBuffer.getDataType(), dumpedBuffer.isCompressed());
      }
    }
  }
  ...
  public void drain() {
    if (cachedBuffer != null) {
      ...
      try {
        handleRipeBuffer(cachedBuffer, currentSubIdx, cachedBuffer.getDataType(), cachedBuffer.isCompressed());
  ...
  private void handleRipeBuffer(
          Buffer buffer, int subIdx,
          Buffer.DataType dataType,
          boolean isCompressed) throws InterruptedException {
  ...
  public static Queue<Buffer> unpack(ByteBuf byteBuf) {
    ...
    try {
      CheckUtils.checkState(byteBuf instanceof Buffer, "Illegal buffer type.");
