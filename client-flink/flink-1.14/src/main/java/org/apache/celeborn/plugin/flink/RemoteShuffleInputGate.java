...
public class RemoteShuffleInputGate extends IndexedInputGate {
  ...
  private MapShuffleClientImpl shuffleClient;
  ...
  private long initShuffleReadClients(int bufferSize) {
    ...
    for (Pair<Integer, ShuffleDescriptor> descriptor : descriptors) {
      ...
      ShuffleResourceDescriptor shuffleDescriptor =
          remoteDescriptor.getShuffleResource().getShuffleResourceDescriptor();
  ...
  // This method will act as buffer supplier
  ...
  private Consumer<ByteBuf> getDataListener(int channelIdx) {
    return byteBuf -> {
      ...
      try {
        // TODO Split bytebuf to buffer queue
