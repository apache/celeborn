...
public interface ClusterTenantConfigMapper {
  ...
  @Update(
      "UPDATE celeborn_cluster_tenant_config SET config_value = #{configValue}, gmt_modify = #{gmtModify} "
          + "WHERE cluster_id = #{clusterId} AND tenant_id = #{tenantId} AND config_key = #{configKey} AND name IS NULL")
  int updateConfig(ClusterTenantConfig clusterTenantConfig);

  @Update(
      "UPDATE celeborn_cluster_tenant_config SET config_value = #{configValue}, gmt_modify = #{gmtModify} "
          + "WHERE cluster_id = #{clusterId} AND tenant_id = #{tenantId} AND name = #{name} AND config_key = #{configKey}")
