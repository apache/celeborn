...
private[api] class ConfResource extends ApiRequestContext {
  ...
  private def upsertDynamicConfig(request: UpsertDynamicConfigRequest): Unit = {
    ...
    if (ConfigLevel.SYSTEM.equals(level)) {
      configService.upsertSystemConfig(request.getConfigs)
    } else if (ConfigLevel.TENANT.equals(level) || ConfigLevel.TENANT_USER.equals(level)) {
      configService.upsertTenantConfig(request.getTenant, request.getName, request.getConfigs)
    }
  }

  private def deleteDynamicConfig(request: DeleteDynamicConfigRequest): Unit = {
    val level = request.getLevel
    if (ConfigLevel.SYSTEM.equals(level)) {
      configService.deleteSystemConfigByKeys(request.getConfigs)
    } else if (ConfigLevel.TENANT.equals(level) || ConfigLevel.TENANT_USER.equals(level)) {
      configService.deleteTenantConfigByKeys(request.getTenant, request.getName, request.getConfigs)
    }
  }
}
