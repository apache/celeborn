...
|   spark.rss.client.compression.codec    |   lz4   |                                                                   Compression and decompression codec used by RSS shuffle data. Support `lz4` and `zstd` codec, and input parameter will be converted to lowercase.                                                                           |
| spark.rss.client.compression.zstd.level |    1    |                                The compression degree of zstd, the value is limited to an integer between -5 to 22. The lower the level, the faster the speed,thus there is a trade-off between the compression ratio and the compression speed.                                              |
