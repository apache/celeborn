...
|  `rss.worker.checkIfFileCleanedRetryCount` |                  3                   |   int   |               The number of retries for a worker to check if the working directory is cleaned up before registering with the master.            |
|  `rss.worker.checkIfFileCleanedTimeoutMs`  |                 1000                 |   long  |              The wait time per retry for a worker to check if the working directory is cleaned up before registering with the master.           |
