...
| `rss.worker.memory.direct.critical.ratio` | 0.9 | float | Worker direct memory usage critical level ratio |
| `rss.worker.memory.check.interval` | 10 | String | Timeunit is millisecond |
| `rss.worker.channel.limiter.enabled` | true | bool | Flow control |
| `ess.worker.memory.report.interval` | 10s | String | Timeunit is second |
